...
def test_exon_skipping_result_has_normalized_input_and_provenance():
    result = design_exon_skipping(request("T" * 98 + "AG", "A" * 50, "C" * 100))

    assert result.schema_version == "1.0"
    assert result.application == "exon_skipping"
    assert result.normalized_input.upstream_intron.endswith("AG")
    assert result.normalized_input.exon == "A" * 50
    assert result.normalized_input.coordinate_system == "concatenated_sequence_1_based"
    assert result.provenance.engine.name == "leaper-arrna-engine"
    assert result.provenance.ruleset.id == "leaper-exon-skipping"
    assert result.provenance.ruleset.version == "0.1.0"
