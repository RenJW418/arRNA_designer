  ...
  return (
    <aside className="preview-panel">
      <div className="preview-header">
        ...
        <span>{sourceLabel ?? (mode === "gene" ? "NCBI reference" : "User sequence")}</span>
      </div>
      {mode === "gene" ? (
        <>
        ...
        <>
          <div className="sequence-window-meta">
            <span>{start + 1}</span>
            <strong>{context.length} nt window</strong>
            <span>{end}</span>
          </div>
          ...
          <p className="preview-help">
            Showing up to 76 nt on each side of position {safePosition}, 153 nt in total. Click a colored A to select it.
          </p>
        </>
      ) : (
        <div className="empty-preview">
          <span>ACGT</span>
          <p>Your in-frame sequence context will appear here.</p>
        </div>
      )}
      <div className="legend editing-legend">
        <span><i className="legend-target" /> Selected site</span>
        <span><i className="legend-editable" /> Editable A</span>
        <span><i className="legend-synonymous" /> Synonymous A</span>
        <span><i className="legend-blocked" /> GA, difficult to edit</span>
      </div>
    </aside>
  );
}
